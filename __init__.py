...
### This version is a modified fork of Avastar 1, adapted for Blender 5.0+
###
...
if "bpy" in locals():
    ...
    if "propgroups" in locals():
        importlib.reload(propgroups)
    if "animation" in locals():
        importlib.reload(animation)
    if "armature_util" in locals():
        importlib.reload(armature_util)
    if "create" in locals():
        importlib.reload(create)
    if "context_util" in locals():
        importlib.reload(context_util)
    if "data" in locals():
...
class AVASTAR_MT_TemplatesMenu(bpy.types.Menu):
    ...
    def draw(self, context):
        from .const import get_user_resource
        blender_scripts = get_user_resource('SCRIPTS', path="presets")
...
def register():
    ...
    if True:  # oder eine echte Bedingung wie get_blender_revision() < 279000
        ...
        path_app_templates = get_user_resource(
            'SCRIPTS'
        )
        path_app_templates = os.path.join(path_app_templates, "startup", "bl_app_templates_user")
